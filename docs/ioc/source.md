# 源代码提交页（智能楼宇IOC运营系统 buildingos.ioc）
...
const SVG_ICONS = {
  alarm:
    '<svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M12 2L1 21h22L12 2zm0 4l7.53 13H4.47L12 6zm-1 8h2v2h-2v-2zm0-6h2v4h-2V8z"/></svg>',
  pending:
  ...
  cameraOnline:
    '<svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',
  cameraOffline:
    '<svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>',
  personnel:
  ...
  patrol:
    '<svg viewBox="0 0 24 24" width="28" height="28" fill="currentColor"><path d="M15.5 5.5c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zM5 12c-2.8 0-5 2.2-5 5s2.2 5 5 5 5-2.2 5-5-2.2-5-5-5zm0 8.5c-1.9 0-3.5-1.6-3.5-3.5s1.6-3.5 3.5-3.5 3.5 1.6 3.5 3.5-1.6 3.5-3.5 3.5zm9.8-1.8l-3.9-3.9 1.4-1.4 2.5 2.5 5.1-5.9 1.5 1.3-6.6 7.4zM10.9 5.5c-.5-.5-1.1-.8-1.7-.9L9 2h4l-.2 2.6c-.6.1-1.2.4-1.7.9l-2.5 2.5 2.5 2.5c.5.5 1.1.8 1.7.9l.2 2.6H9l.2-2.6c-.6-.1-1.2-.4-1.7-.9l-2.5-2.5 2.5-2.5z"/></svg>',
}
...
const loadData = async () => {
  try {
    ...
    summaryCards.value = [
      ...
      {
        label: '在线摄像头',
        value: overview.onlineCameras,
        color: '#67C23A',
        icon: SVG_ICONS.cameraOnline,
      },
      {
        label: '离线摄像头',
        value: overview.offlineCameras,
        color: '#909399',
        icon: SVG_ICONS.cameraOffline,
      },
      {
        label: '安保人员',
        value: overview.securityPersonnel,
        color: '#409EFF',
        icon: SVG_ICONS.personnel,
      },
      {
        label: '巡逻路线',
        value: overview.patrolRoutes,
        color: '#B37FEB',
        icon: SVG_ICONS.patrol,
      },
    ]
...
const areaOptions = [
  ...
  { value: 'B栋', label: 'B栋' },
  { value: 'C栋', label: 'C栋' },
  { value: '园区户外', label: '园区户外' },
  { value: '地下车库', label: '地下车库' },
]

const typeOptions = [
  { value: '', label: '全部' },
  { value: '入侵检测', label: '入侵检测' },
  { value: '火灾预警', label: '火灾预警' },
  { value: '门禁异常', label: '门禁异常' },
  { value: '设备故障', label: '设备故障' },
  { value: '周界报警', label: '周界报警' },
]
...
const trendOption = computed(() => ({
  ...
  series: [
    ...
    },
    {
      name: '已处理数',
      type: 'line',
      data: trendData.value.map((d) => d.handled),
      smooth: true,
  ...
  series: [
    {
      ...
      data: areaData.value.map((d, i) => ({
        name: d.area,
        value: d.count,
        itemStyle: { color: areaColors[i] },
      })),
    },
  ],
}))
...
const fetchOverview = async () => {
  try {
    const { data } = await getAlarmOverview()
    overview.value = data
  } catch {
    baseMessage.error('获取告警概览失败')
  }
}

const fetchTrend = async () => {
  try {
    const { data } = await getAlarmTrendWeekly()
    trendData.value = data
  } catch {
    baseMessage.error('获取趋势数据失败')
  }
}

const fetchSource = async () => {
  try {
    const { data } = await getAlarmBySource()
    sourceData.value = data
  } catch {
    baseMessage.error('获取来源数据失败')
  }
}

const fetchArea = async () => {
  try {
    const { data } = await getAlarmByArea()
    areaData.value = data
  } catch {
    baseMessage.error('获取区域数据失败')
  }
}
...
const fetchAllData = async () => {
  loading.value = true
  await Promise.all([fetchOverview(), fetchTrend(), fetchSource(), fetchArea(), fetchAlarmList()])
  loading.value = false
}
      ...
      <el-col :span="6">
        <el-card class="summary-card" shadow="hover">
          <div class="summary-card-body">
            ...
            <div class="summary-icon vehicle-flow">
              <vab-icon icon="car-line" />
            </div>
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :span="6">
        <el-card class="summary-card" shadow="hover">
          <div class="summary-card-body">
            ...
            <div class="summary-icon visitor">
        ...
        <el-card class="parking-card" shadow="hover">
          ...
          <div class="parking-content">
            ...
            <div class="parking-summary">
              <span class="parking-text">
                已用
                <strong>{{ overview.parkingTotal - overview.parkingAvailable }}</strong>
                / 总
                <strong>{{ overview.parkingTotal }}</strong>
                车位
              </span>
...
defineOptions({
  name: 'OperationsPassage',
})

const $baseMessage = inject<any>('$baseMessage') || ElMessage

// ── Types ──
interface PassageOverview {
  todayVehicleFlow: number
  vehicleFlowChange: number
...
const overview = ref<PassageOverview>({
  todayVehicleFlow: 0,
  vehicleFlowChange: 0,
  parkingOccupancy: 0,
  parkingTotal: 0,
...
// ── Parking Area Breakdown (derived from overview totals) ──
const parkingAreas = computed<ParkingArea[]>(() => {
  const total = overview.value.parkingTotal
  const used = total - overview.value.parkingAvailable
  // Distribute 390 used across 4 areas in realistic proportions
  ...
  return areaData.map((a) => ({
    ...a,
    used: Math.round(a.used * scale),
    total: Math.round(a.total * scale),
    rate: Math.round((a.used / a.total) * 100),
  }))
})
  ...
  return {
    tooltip: {
      ...
      formatter: (params: any[]) => {
        ...
        let html = `<strong>${hour}</strong><br/>`
    ...
    xAxis: {
      type: 'category',
      data: data.map((d: VehicleTrendItem) => d.hour),
    ...
    series: [
      {
        name: '入场',
        type: 'bar',
        data: data.map((d: VehicleTrendItem) => d.enter),
        barWidth: '35%',
        barGap: '20%',
        itemStyle: {
          color: '#67C23A',
      ...
      },
      {
        name: '出场',
        type: 'bar',
        data: data.map((d: VehicleTrendItem) => d.exit),
        barWidth: '35%',
        itemStyle: {
          color: '#F56C6C',
          borderRadius: [4, 4, 0, 0],
...
const densityOption = computed(() => {
  const data = densityData.value
  if (!data.length) return {}

  // Sort ascending so the chart shows lowest at top
  ...
  return {
    tooltip: {
      ...
      formatter: (params: any[]) => {
        ...
        return `<strong>${row.area}</strong><br/>人数: ${row.count} 人<br/>密度: ${row.density}%`
      },
    },
    grid: {
      left: '3%',
    ...
    yAxis: {
      type: 'category',
      data: sorted.map((d: DensityItem) => d.area),
      axisLabel: { fontSize: 12 },
    },
    series: [
      {
        type: 'bar',
        data: sorted.map((d: DensityItem) => ({
          value: d.density,
          itemStyle: {
...
const parkingOption = computed(() => {
  const total = overview.value.parkingTotal
  const available = overview.value.parkingAvailable
  const used = total - available

  return {
    tooltip: {
      trigger: 'item',
      formatter: (params: any) => `${params.name}: ${params.value} 个 (${params.percent}%)`,
    },
    series: [
      {
        type: 'pie',
...
const fetchData = async () => {
  loading.value = true
  try {
    const [overviewRes, trendRes, densityRes, recordsRes] = await Promise.all([
      getPassageOverview(),
      getVehicleTrend(),
      getPeopleDensityByArea(),
      getRecentVehicleRecords(),
    ])
    overview.value = overviewRes.data
    vehicleTrend.value = trendRes.data
    densityData.value = densityRes.data
    vehicleRecords.value = recordsRes.data
  } catch (error: any) {
    $baseMessage?.error(error?.msg || '获取通行看板数据失败')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="environment-dashboard">
    ...
    <el-card shadow="hover" class="section-card">
      <template #header>
        <span class="section-title">各区域温度趋势</span>
      </template>
    ...
    <el-card shadow="hover" class="section-card">
      <template #header>
        <span class="section-title">环境传感器详情</span>
      </template>
      <el-table v-loading="tableLoading" :data="sensors" border stripe style="width: 100%">
        ...
        <el-table-column label="温度" width="110">
          <template #default="{ row }">
            <span :class="{ 'cell-hot': row.temp > 26 }">
              <span class="inline-dot" :style="{ background: row.temp > 26 ? '#F56C6C' : '#409EFF' }" />
              {{ row.temp }}°C
            </span>
          </template>
        </el-table-column>
        <el-table-column label="湿度" width="100">
          <template #default="{ row }">
            <span class="inline-dot" style="background: #36d399" />
            {{ row.humidity }}%
          </template>
        </el-table-column>
        <el-table-column label="PM2.5" min-width="180">
          <template #default="{ row }">
            <div class="sensor-cell">
              ...
              <el-progress
                :percentage="Math.min(100, Math.round((row.pm25 / 75) * 100))"
                :color="row.pm25 > 40 ? '#E6A23C' : '#67C23A'"
                :stroke-width="6"
        ...
        <el-table-column label="CO₂" min-width="180">
          <template #default="{ row }">
            <div class="sensor-cell">
              ...
              <el-progress
                :percentage="Math.min(100, Math.round((row.co2 / 2000) * 100))"
                :color="row.co2 > 800 ? '#E6A23C' : '#67C23A'"
                :stroke-width="6"
                style="flex: 1; margin-left: 8px"
              />
            </div>
        ...
        <el-table-column label="噪音" width="100">
          <template #default="{ row }">{{ row.noise }}dB</template>
        </el-table-column>
...
const chartOption = computed(() => {
  ...
  const series = t.areas.map((area) => ({
    ...
    itemStyle: { color: areaColors[area] || '#409EFF' },
    symbol: 'circle',
    symbolSize: 6,
    areaStyle: {
      color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
        { offset: 0, color: `${areaColors[area] || '#409EFF'}30` },
        { offset: 1, color: `${areaColors[area] || '#409EFF'}05` },
      ]),
    },
  }))
      ...
      formatter(params: any) {
        ...
        params.forEach((p: any) => {
          html += `<div style="display:flex;align-items:center;gap:6px;margin:2px 0">
            ...
            <span>${p.seriesName}：<b>${p.value}°C</b></span>
          </div>`
        })
        return html
      },
...
async function loadData() {
  try {
    const [overviewRes, trendRes, sensorsRes] = await Promise.all([
    ...
    trend.value = trendRes.data ?? trendRes

    tableLoading.value = true
    sensors.value = sensorsRes.data ?? sensorsRes
  } catch {
    // Dashboard pages degrade gracefully on fetch failure
  } finally {
    tableLoading.value = false
  }
}
          ...
          <div v-if="card.showProgress" class="card-progress" :style="{ color: card.color }">
            <svg viewBox="0 0 36 36" width="52" height="52">
              ...
              <path
                ...
                :stroke-dashoffset="100 - (card.rawValue || 0)"
                stroke-linecap="round"
              />
              <text x="18" y="20.5" text-anchor="middle" font-size="8" :fill="card.color" font-weight="bold">
                {{ card.rawValue || card.progressValue }}%
              </text>
            </svg>
          </div>
        </div>
        ...
        <vab-card title="今日菜单" class="menu-card">
          ...
          >
            ...
            <el-table-column prop="category" label="分类" width="70">
              <template #default="{ row }">
                <el-tag :type="tagType(row.category)" size="small" effect="plain">
                  {{ row.category }}
                </el-tag>
              </template>
            </el-table-column>
...
// ── Helpers ──
const tagType = (category: string): string => {
  const map: Record<string, string> = {
    中餐: 'danger',
    西餐: 'warning',
    面点: 'success',
...
const onSortChange = (sort: { prop: string; order: string }) => {
  if (sort.prop === 'sales' && sort.order) {
    const sorted = [...todayMenu.value]
    sorted.sort((a, b) => (sort.order === 'ascending' ? a.sales - b.sales : b.sales - a.sales))
    todayMenu.value = sorted
  }
}

const inPeakHours = (hour: string): boolean => {
  return hour >= peakHoursRange.start && hour <= peakHoursRange.end
}

// ── Build ECharts Options ──
const buildDiningTrendOption = (data: any[]) => {
  const hours = data.map((d: any) => d.hour)
  ...
  return {
    tooltip: {
      ...
      axisPointer: { type: 'cross' },
      formatter: (params: any[]) => {
        const bar = params.find((p: any) => p.seriesName === '就餐人数')
        const line = params.find((p: any) => p.seriesName === '趋势线')
        let html = `<strong>${bar?.axisValue || ''}</strong><br/>`
        if (bar) html += `就餐人数: ${bar.value}<br/>`
        if (line) html += `趋势: ${line.value}`
        return html
      },
    },
    grid: {
    ...
    series: [
      {
        name: '就餐人数',
        type: 'bar',
        data: counts.map((count: number, idx: number) => ({
          value: count,
          itemStyle: {
            color: inPeakHours(hours[idx])
...
const buildAreaDistributionOption = (data: any[]) => {
  const areas = data.map((d: any) => d.area)
  const counts = data.map((d: any) => d.count)
  ...
  return {
    tooltip: {
      ...
      formatter: (params: any[]) => {
        ...
        const item = data[idx]
        if (!item) return ''
        return `<strong>${item.area}</strong><br/>就餐人数: ${item.count}<br/>餐位总数: ${item.total}<br/>占用率: ${item.rate}%`
      },
    },
    legend: {
      data: ['就餐人数', '占用率'],
    ...
    series: [
      ...
      },
      {
        ...
        data: rates.map((r: number) => ({
          value: r,
          itemStyle: { color: 'rgba(64, 158, 255, 0.25)', borderRadius: [0, 3, 3, 0] },
        })),
        barMaxWidth: 16,
        barGap: '-100%',
        z: 0,
    ...
    series: [
      {
        name: '早餐',
        type: 'bar',
        data: data.map((d: any) => d.breakfast),
...
const loadData = async () => {
  try {
    const [overviewRes, trendRes, areaRes, weeklyRes, menuRes] = await Promise.all([
      getRestaurantOverview(),
      getDiningTrend(),
      getDiningAreaDistribution(),
      getWeeklyDiningStats(),
      getTodayMenu(),
    ])
    ...
    summaryCards.value = [
      {
        label: '当前就餐人数',
        value: `${overview.currentDiners}`,
        rawValue: overview.currentDiners,
        progressValue: '',
        color: '#409EFF',
        showProgress: false,
      },
      {
        label: '座位占用率',
        value: `${overview.seatingOccupancy}%`,
        rawValue: overview.seatingOccupancy,
        progressValue: overview.seatingOccupancy,
        color: '#67C23A',
        showProgress: true,
      },
      {
        label: '今日用餐总数',
        value: `${overview.todayTotalMeals}`,
        rawValue: overview.todayTotalMeals,
        progressValue: '',
        color: '#B37FEB',
        showProgress: false,
      },
      {
        label: '平均排队时间',
        value: `${overview.avgQueueTime}min`,
        rawValue: overview.avgQueueTime,
        progressValue: '',
        color: '#E6A23C',
        showProgress: false,
      },
    ...
    // Today menu (sort by sales desc by default)
        ...
        <div class="summary-card" :style="{ borderLeftColor: card.color }">
          <div class="card-content">
            <div class="card-value" :style="{ color: card.color }">
              <span v-if="card.pulse" class="pulse-dot" />
              {{ card.value }}
        ...
        <vab-card title="会议室实时状态" class="room-card">
          <div class="room-grid">
            <div v-for="room in roomUsageList" :key="room.room" class="room-item" :class="'status-' + room.status">
              ...
              <div v-if="room.meeting" class="room-meeting">
                {{ room.meeting }}
                <span v-if="room.until">至{{ room.until }}</span>
              </div>
            </div>
          </div>
        </vab-card>
        ...
        <vab-card title="当前进行会议" class="table-card">
          <el-table :data="currentMeetings" style="width: 100%" size="small" :show-header="true" max-height="240" stripe>
            ...
            <el-table-column label="时间" min-width="110">
              <template #default="{ row }">
                <el-tag size="small" effect="plain" type="primary" style="font-size: 11px">{{ row.startTime }}-{{ row.endTime }}</el-tag>
              </template>
            </el-table-column>
...
const loadData = async () => {
  try {
    const [overviewRes, roomUsageRes, deptStatsRes, meetingsRes, weeklyTrendRes] = await Promise.all([
      getMeetingOverview(),
      getRoomUsage(),
      getMeetingDeptStats(),
      getCurrentMeetings(),
      getMeetingTrendWeekly(),
    ])

    const overview = overviewRes.data ?? overviewRes
    const roomUsage = roomUsageRes.data ?? roomUsageRes
    const deptStats = deptStatsRes.data ?? deptStatsRes
    const meetings = meetingsRes.data ?? meetingsRes
    const weeklyTrend = weeklyTrendRes.data ?? weeklyTrendRes

    // Populate summary cards
    summaryCards.value = [
      {
        label: '今日会议数',
        value: overview.todayMeetings,
        color: '#409EFF',
      },
      {
        label: '进行中',
      ...
      {
        label: '平均时长',
        value: overview.avgMeetingDuration,
        color: '#909399',
        unit: '分钟',
      },
    ]
    ...
    // Room usage
    roomUsageList.value = roomUsage

    // Current meetings
    currentMeetings.value = meetings.slice(0, 5)

    // Dept Stats option
    deptStatsOption.value = buildDeptStatsOption(deptStats)

    // Weekly Trend option
    weeklyTrendOption.value = buildWeeklyTrendOption(weeklyTrend)
  } catch (err) {
    const msg = $baseMessage?.error ?? ElMessage.error
    msg('加载会议看板数据失败')
    console.error(err)
...
const buildDeptStatsOption = (data: any[]) => {
  // Reverse so highest values appear at top
...
const buildWeeklyTrendOption = (data: any[]) => ({
  ...
  series: [
    ...
    },
    {
      ...
      type: 'line',
      yAxisIndex: 1,
      data: data.map((d: any) => d.usageRate),
      smooth: true,
      lineStyle: { color: '#67C23A', width: 2 },
      itemStyle: { color: '#67C23A' },
      symbol: 'circle',
